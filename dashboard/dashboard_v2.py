...
with st.sidebar:
    ...
    # Sélection de zone
    zones = ["Toutes les zones", "centre", "maarif", "ain-diab", "sidi-maarouf"]
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["💨 Qualité de l'air", "🚗 Trafic", "⚡ Énergie", "🔊 Bruit", "🗺️ Vue d'ensemble", "👥 Citoyens"])
...
with tab5:
    ...
    if not df_air.empty and not df_traffic.empty:
        ...
        zones_list = ["centre", "maarif", "ain-diab", "sidi-maarouf"]
    ...
    summary_data = []
    for zone in ["centre", "maarif", "ain-diab", "sidi-maarouf"]:
...
# ── Tab 6 : Citoyens ──────────────────────────────────────────────────────────
with tab6:
    df_agents = load_data("agents", n_points)

    if selected_zone != "Toutes les zones":
        df_agents = df_agents[df_agents["zone"] == selected_zone]
    ...
    if not df_agents.empty:
        ...
        with c1:
            total_pop = int(df_agents.groupby("zone")["population"].last().sum())
            st.metric("👥 Population simulée", total_pop)
        with c2:
            total_cars = int(df_agents.groupby("zone")["using_car"].last().sum())
            st.metric("🚗 Voitures actives", total_cars)
        with c3:
            total_home = int(df_agents.groupby("zone")["stayed_home"].last().sum())
            st.metric("🏠 Restés (pollution)", total_home,
                      delta="⚠️ Pollution élevée" if total_home > 5 else "✅ Normal",
                      delta_color="inverse" if total_home > 5 else "normal")
        with c4:
            total_energy = round(df_agents["energy_kw"].sum(), 1)
            st.metric("⚡ Énergie citoyens", f"{total_energy} kW")

        st.divider()
        col1, col2 = st.columns(2)

        with col1:
            # Graphe activités dans le temps
            fig = px.line(df_agents, x="timestamp",
                          y=["at_home", "at_work", "in_transport", "in_commercial"],
                          color_discrete_map={
                              "at_home":       "#89b4fa",
                              "at_work":       "#a6e3a1",
                              "in_transport":  "#fab387",
                              "in_commercial": "#f38ba8"
                          },
                          title="Activités des citoyens dans le temps",
                          labels={"value": "Citoyens", "timestamp": "Heure", "variable": "Activité"})
            fig.update_layout(template="plotly_dark", height=350)
            st.plotly_chart(fig, use_container_width=True)

        with col2:
            # Voitures vs stayed_home par zone
            latest_agents = load_latest("agents")
            if not latest_agents.empty:
                fig2 = px.bar(latest_agents, x="zone",
                              y=["using_car", "stayed_home"],
                              title="Voitures actives vs Restés à domicile (pollution)",
                              barmode="group",
                              color_discrete_map={
                                  "using_car":   "#fab387",
                                  "stayed_home": "#f38ba8"
                              },
                              labels={"value": "Citoyens", "zone": "Zone"})
                fig2.update_layout(template="plotly_dark", height=350)
                st.plotly_chart(fig2, use_container_width=True)

        # Corrélation PM2.5 vs stayed_home — le graphe CPS² clé
        st.subheader("🔗 Corrélation CPS² — Pollution réelle → Comportement social")
        st.caption("Ce graphe montre l'interaction entre données physiques (PM2.5) et comportements sociaux (citoyens restés chez eux)")

        if not df_air.empty and not df_agents.empty:
            # Merge air + agents par zone et timestamp
            df_air_last    = df_air.groupby("zone")["pm25"].mean().reset_index()
            df_agents_last = df_agents.groupby("zone")["stayed_home"].mean().reset_index()
            df_corr        = df_air_last.merge(df_agents_last, on="zone")

            fig3 = px.scatter(df_corr, x="pm25", y="stayed_home",
                              text="zone", size_max=20,
                              color="zone",
                              color_discrete_map={
                                  "centre":       "#89b4fa",
                                  "maarif":       "#a6e3a1",
                              ...
                              title="PM2.5 (physique) vs Citoyens restés chez eux (social)",
                              labels={"pm25": "PM2.5 µg/m³", "stayed_home": "Citoyens restés"})
            fig3.update_traces(textposition="top center")
            fig3.update_layout(template="plotly_dark", height=400)
            st.plotly_chart(fig3, use_container_width=True)

        # Tableau récapitulatif citoyens
        st.subheader("État actuel des citoyens par zone")
        latest_ag = load_latest("agents")
        if not latest_ag.empty:
            st.dataframe(
                latest_ag[["zone", "population", "at_home", "at_work",
                            "in_transport", "in_commercial", "using_car",
                            "stayed_home", "energy_kw"]],
                use_container_width=True,
                hide_index=True
            )
    else:
        st.info("⏳ En attente des données agents — lance citizen_publisher.py")
